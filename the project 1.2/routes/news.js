...
//   });
// });
router.post('/', [auth, admin], upload.single('imagelink'), async (req, res) => {
  if (!req.file)
      return res.status(400).send("you didn't upload image (jpeg OR png)")
  let {err} = validate(Object.assign({}, req.body, { imagelink: req.file.path }));
  if (err)
  ...
  req.file.path = req.file.path.replace("/images/", "/");

  news = new New/(_.pick(req.body, ['new','author','imagelink']));
      ({
          new: req.body.new,
          author: req.body.author,
          imagelink: (process.env.BASE_URL || config.get('BASE_URL')) + req.file.path,
          filespath: req.file.path
      })    
    if(error)return res.status(400).send(error.message);
    let news= new New(_.pick(req.body, ['new','author']));
    res.status(200).send(_.pick(news,['new','author'])); 
